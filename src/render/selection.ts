...
export type SelectionEvent =
  ...
  /**
   * arrow / note / supporting-text onFocus. PRIMARY, but only reachable from
   * the keyboard: every sender is gated on `shouldFocusSelect`, so pointer
   * focus (which fires before a click and once clobbered modifier-clicks)
   * never dispatches it. Selecting an already-selected key is a no-op so
   * tabbing through a multi-selection does not collapse it.
   */
...
export function selectionReducer(
  ...
  switch (event.type) {
    ...
    case 'focus/reveal':
      return state.keys.includes(event.key) ? state : replace(event.key)
    case 'panel/rowFocus':
